SUBROUTINE refresh_artn()
  ...
  !! overwrite data in artn_params with data in artn_data_ptr
  !!-----------------------------------

  !! integer
  write(*,*) "ninit"
  write(*,*) ninit
  write(*,*) artn_data_ptr% ninit
  ...
  write( iunartout,*) "new header:"
    ...
    WRITE (iunartout,'(15X,"lanczos_at_min          = ", L)') lanczos_at_min
    WRITE (iunartout,'(5X, "--------------------------------------------------")')
    WRITE (iunartout,'(/,/)')

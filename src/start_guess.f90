...
SUBROUTINE start_guess( idum, nat, push, eigenvec )
  ...
  SELECT CASE( TRIM(push_mode) )
    ...
    CASE( 'all', 'list', 'rad' )
       ...
       IF( lUSER_CHOOSE_PER_ATOM )THEN
         ...
         !CALL push_init( nat, tau_step, order, lat, idum, push_ids, push_dist_thr, push_add_const, push_step_size_per_atom, push, push_mode)
         CALL push_init( nat, tau_step, lat, idum, push_ids, push_dist_thr, push_add_const, &
                         push_step_size_per_atom, push, push_mode )
       ELSE
         !CALL push_init( nat, tau_step, order, lat, idum, push_ids, push_dist_thr, push_add_const, push_step_size, push, push_mode)
         CALL push_init( nat, tau_step, lat, idum, push_ids, push_dist_thr, push_add_const, push_step_size, push, push_mode)
       ENDIF
       !
    CASE( 'file' )
       ! 
  ...
  ELSE
    ...
    call push_init( nat, tau_step, lat, idum, mask, push_dist_thr, push_add_const, eigen_step_size, eigenvec, 'list_force')

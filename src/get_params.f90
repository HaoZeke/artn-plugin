...
contains
  ...
  module subroutine get_param_real( name, val, ierr )
    ...
    ierr = 0
    select case( name )
    case( "push_dist_thr"           ); val = push_dist_thr
    case( "forc_thr"                ); val = unconvert_force( forc_thr )
    case( "eigval_thr"              ); val = unconvert_hessian( eigval_thr )
    case( "delr_thr"                ); val = delr_thr
    case( "lanczos_eval_conv_thr"   ); val = lanczos_eval_conv_thr
    case( "push_step_size"          ); val = unconvert_length( push_step_size )
    case( "push_step_size_per_atom" ); val = unconvert_length( push_step_size_per_atom )
    case( "lanczos_disp"            ); val = unconvert_length( lanczos_disp )
    case( "eigen_step_size"         ); val = unconvert_length( eigen_step_size )
    case( "etot_diff_limit"         ); val = unconvert_energy( etot_diff_limit )
    case( "alpha_mix_cr"            ); val = alpha_mix_cr
    case default
       ierr = ERR_VARNAME

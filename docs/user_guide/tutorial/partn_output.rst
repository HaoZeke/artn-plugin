...
Whenever the value of :doc:`../../params/verbose` parameter is nonzero: ``verbose > 0``, pARTn produces several output files in a single run:
...
For details on how to troubleshoot a calculation see :ref:`troubleshoot` page.
...
.. note::
   ...
   The structure files are only produced when the value of ``struc_format_out`` is **not** ``'none'``.

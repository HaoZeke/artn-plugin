...
if(NOT LUA_FOUND)
    message(
        FATAL_ERROR
        "pARTn :: Lua library (liblua) needed for Siesta, not found."
    )
endif()
...
if(NOT EXISTS "${SIESTA_PATH}/CMakeCache.txt")
    message(
        FATAL_ERROR
        "pARTn :: Incorrect Siesta build path (SIESTA_PATH): ${SIESTA_PATH}."
    )
endif()
...
if(NOT "${CMAKE_MATCH_1}" STREQUAL "ON")
    message(
        FATAL_ERROR
        "pARTn :: Siesta needs to be configured with -DSIESTA_WITH_FLOOK=ON"
    )
endif()

## libpartn_lua.so library
add_library(artn_lua SHARED)
target_sources(
    artn_lua
    PRIVATE
...
target_link_libraries(artn_lua PUBLIC MPI::MPI_Fortran)
## link to artn lib
target_link_libraries(artn_lua PUBLIC artn)
## link lua lib
target_link_libraries(artn_lua PRIVATE ${LUA_LIBRARIES})
...
## copy libpartn_lua.so to artn/lib
## <TARGET_FILE:project> is full path name of target; and <TARGET_FILE_NAME:project> is just filename
add_custom_command(
    TARGET artn_lua
    POST_BUILD
    COMMAND
        ${CMAKE_COMMAND} -E copy "$<TARGET_FILE:artn_lua>"
        "${CMAKE_CURRENT_SOURCE_DIR}/lib/$<TARGET_FILE_NAME:artn_lua>"
)

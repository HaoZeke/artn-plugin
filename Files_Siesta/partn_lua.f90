module partn_lua
  ...
  contains
    ...
    function printstruc( lua )result(nret)bind(C, name="printstruc")
      ...
      open( unit = 456, file = "conf.xyz", status = "unknown",  position = "append")
      n = lua_tonumber( lua, -1)
      nat = int(n)
      call lua_pop(lua, 1)
      ...
      write(456,*) nat
      write(456,*) 'Lattice="',lat,'" properties=species:I:1:pos:R:3:forces:R:3:Force_magnitude:R:1'
      do i = 1, nat
         write(456,*) typ(i), coords(:,i), force(:,i), norm2(force(:,i))
      end do
    ...
    ! function set1d(lua)result(nret)bind(c)
    !   type( c_ptr ), intent(in), value :: lua
    !   integer( c_int ) :: nret
    ...
    !   allocate( a(1:n) )
    !   call receive_1D_arr( lua, N, a)

    !   call send_1D_arr( lua, N, a)
    !   deallocate( a )
    !   nret = 1
    ! end function set1d

    ! function set2d(lua)result(nret)bind(c)
    !   type( c_ptr ), intent(in), value :: lua
    !   integer( c_int ) :: nret
    ...
    !   allocate( a2(1:3,1:n) )
    !   call receive_2D_arr( lua, 3, N, a2)

    !   !! send back array a2 as first result
    !   call send_2D_arr( lua, 3, N, a2)

    !   allocate( a3, source=a2)
    !   a3(1,1)=99.9

    !   !! send back array a3 as second result
    !   call send_2d_arr(lua, 3, N, a3)

    !   deallocate( a2 )
    !   deallocate(a3)

    !   !! we're sending two results
    !   nret = 2
    ! end function set2d
    ! !!=============================================

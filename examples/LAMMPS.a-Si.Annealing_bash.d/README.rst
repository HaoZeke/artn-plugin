...
Each group will do its own searches independently with ``lammps`` in its own diectory. 
The number of events to be searched by each group is defined in the ``do`` loop of the ``lammps.in`` file.

In accordance with your system, you can modify the ``artn.in`` file in which the initial push of the exploration is specified.

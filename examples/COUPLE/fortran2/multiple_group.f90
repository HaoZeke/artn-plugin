program single
  ...
  call mpi_comm_rank( MPI_COMM_WORLD, orig_rank, ierr )

  !! set group index
  igroup = mod( orig_rank+ngroup, ngroup )
  me = orig_rank

  call mpi_comm_split( MPI_COMM_WORLD, igroup, me, gcomm, ierr )
  call mpi_comm_rank( gcomm, group_rank, ierr )

  write(*,'(3(a,1x,i2,1x))') "me",me, "group:",igroup, "group_rank:",group_rank

  !! init lammps
  lmp = lammps(args, comm=gcomm )
  !! init partn
  ! artn = t_partn()
  ...
  if( igroup == 2) addconst(:,2) = [0.0, 0.0, -1.0, 10.0]
  if( igroup == 3) addconst(:,2) = [1.0, 1.0, 0.0, 10.0]

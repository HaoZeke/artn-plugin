...
echo "QE_MPIF90" "${QE_MPIF90}" "${mpif90}" >& AS_MESSAGE_FD
if test x"${QE_MPIF90}" != x"${mpif90}"; then
  ...
  AC_PROG_FC( ${QE_MPIF90} )
  if test x"$FC" = x""; then
  dnl ## QE_F90 compiler was not found
  AC_MSG_ERROR(
  [Unable to set MPIF90=${QE_MPIF90}. Check your loaded modules, or re-configure QE to: MPIF90=${FC_OLD}.],
  -1)
  fi
...
if test x"${QE_F90}" != x"${f90}"; then
  FC_OLD=${f90}
  unset FC f90 ac_cv_prog_ac_ct_FC ac_cv_fc_compiler_gnu ac_cv_prog_fc_g ac_cv_fc_libs
  AC_MSG_WARN([Overloading F90 due to QE, old=${FC_OLD} new=${QE_F90}])
  AC_PROG_FC( ${QE_F90})
  if test x"$FC" = x""; then
    dnl ## QE_F90 compiler was not found
    AC_MSG_ERROR(
    [Unable to set F90=${QE_F90}. Check your loaded modules, or re-configure QE to: F90=${FC_OLD}.],
    -1)
  fi
  AC_SUBST( f90, ["$FC"] )
fi
...
if test -n x"${QE_LAPACK_LIBS}"; then
